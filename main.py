...
def addcontact():
    ...
    while True:
        num = input(
            "What is the person's phone number? (only digits allowed): ")
        if not num.isdigit():
            print("Please enter only digits")
            continue
        elif len(num) != 10:
            print(
                "Please enter 10-digits phone number with no comma, no spaces, no punctuation")
            continue
        else:
            break
    cursor.execute('''INSERT INTO phonebook (name, surname, phone_number) VALUES (?,?,?)''',
                   (name, surname, num))
    conn.commit()
    print("New contact " + surname + ' ' + name +
          " was added to the phonebook table")


def displaybook():
    cursor.execute(
        "SELECT surname, name, phone_number FROM phonebook ORDER BY surname")
...
def findcontacts():
    s = key_pair_reception('searching')
    if s != None:
        sql = 'SELECT surname, name, phone_number FROM phonebook WHERE ' + \
            s + ' ORDER BY surname'
        cursor.execute(sql)
        results = cursor.fetchall()
        print(results)
